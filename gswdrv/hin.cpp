...
hin::HOOK_HANDLE hin::HookCode(PVOID Module, PBYTE Code, PBYTE NewCode, PVOID *PrevCode, PBYTE Trampoline)
{
...
VOID hin::UnHook(HOOK_HANDLE Handle)
{
	PBYTE Trampoline;
#if defined(_X86_) || defined(_AMD64_)
...
PVOID hin::GetModuleBase(PCHAR Name)
{
...
PVOID hin::GetExportedFunc(PVOID Module, PCHAR Name)
{
...
PVOID hin::GetImportedFunc(PVOID Module, PCHAR Name)
{
	if ( Module == NULL ) return NULL;
...
PVOID hin::MapForWrite(MAPHANDLE *Handle, PVOID Mem, ULONG Length)
{
...
VOID hin::UnMap(MAPHANDLE Handle)
{
#if defined(_NTDDK_) || defined(_NTIFS_)
...
PBYTE hin::AllocateTrampoline(VOID)
{
...
VOID hin::FreeTrampoline(PBYTE Trampoline)
{
...
NTSTATUS hin::HookService(WCHAR *wcServiceProc, ULONG Index, PVOID NewServiceProc, 
                     PVOID *OriginalServiceProc, ServiceType Type) 
{
    ...
    *OriginalServiceProc = InterlockedExchangePointer((PVOID *)virt_addr, NewServiceProc);
...
void hin::UnHookService(WCHAR *wcServiceProc, ULONG Index, PVOID OriginalServiceProc, 
                   PVOID NewServiceProc, ServiceType Type)
{
    ...
    if (OriginalServiceProc != NULL) {
        ...
        InterlockedCompareExchangePointer((PVOID *)virt_addr, OriginalServiceProc, NewServiceProc);
