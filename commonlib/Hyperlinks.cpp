...
std::map<HWND, std::wstring> References;
...
LRESULT CALLBACK _HyperlinkProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_DESTROY:
		{
			std::map<HWND, std::wstring>::iterator i = References.find(hwnd);
			if ( i != References.end() ) References.erase(i);
	...
	case WM_LBUTTONDBLCLK:
		{
			ShellExecute(hwnd, L"open", References[hwnd].c_str(), 0, 0, SW_SHOWMAXIMIZED);
		break;
		}
	case WM_SETCURSOR:
		{
...
BOOL ConvertStaticToHyperlink(HWND hwndCtl, const wchar_t *Reference)
{
	//
	// Add reference to the list
	//
	References[hwndCtl] = Reference;
...
BOOL ConvertStaticToHyperlink(HWND hwndParent, UINT uiCtlId, const wchar_t *Reference)
{
	return ConvertStaticToHyperlink(GetDlgItem(hwndParent, uiCtlId), Reference);
}
